"""Tests for the duckbricks_utils package."""

import os
from unittest.mock import MagicMock, patch

import duckbricks_utils
from duckbricks_utils._connection import _pg_dsn, catalog_name, connect, data_path
...
class TestCatalogName:
    def test_returns_default_when_env_absent(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert catalog_name() == "duckbricks"
...
class TestDataPath:
    def test_returns_default_when_env_absent(self):
        with patch.dict(os.environ, {"DUCKBRICKS_STORAGE_BACKEND": "local"}, clear=False):
    ...
    def test_returns_env_value_when_set(self):
        with patch.dict(os.environ, {"DUCKBRICKS_DATA_PATH": "/custom/path/"}):
            assert data_path() == "/custom/path/"


class TestPgDsn:
    def test_builds_dsn_from_env_vars(self):
        env = {
...
class TestConnect:
    def test_connect_calls_duckdb_and_returns_connection(self):
        mock_conn = MagicMock()
        with (
            patch("duckdb.connect", return_value=mock_conn) as mock_duckdb,
            patch.dict(os.environ, {"DUCKBRICKS_STORAGE_BACKEND": "local"}),
        ):
            result = connect()
    ...
    def test_connect_accepts_data_path_override(self):
        mock_conn = MagicMock()
        with (
            patch("duckdb.connect", return_value=mock_conn),
            patch.dict(os.environ, {"DUCKBRICKS_STORAGE_BACKEND": "local"}),
        ):
            connect(override_data_path="/tmp/custom/")
...
class TestPackagePublicApi:
    def test_public_symbols_are_exported(self):
        assert hasattr(duckbricks_utils, "connect")
        assert hasattr(duckbricks_utils, "catalog_name")
        assert hasattr(duckbricks_utils, "data_path")
